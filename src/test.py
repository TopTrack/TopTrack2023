...
class PrefetchDataset(torch.utils.data.Dataset):
  def __init__(self, opt, dataset, pre_process_func):
    ...
    self.opt = opt
    self.get_default_calib = opt.get_default_calib
  ...
  def __getitem__(self, index):
    ...
    for scale in opt.test_scales:
      ...
      calib = img_info['calib'] if 'calib' in img_info \
        else dataset.get_default_calib(dataset, image.shape[1], image.shape[0])
...
def prefetch_test(opt):
  ...
  for ind, (img_id, pre_processed_images) in enumerate(data_loader):
    ...
    processed_ret = [x for x in ret['results']]
    #processed_ret = [x for x in ret['results'] if x['score'][0] > 0.3]
    
    results[img_id.numpy().astype(np.int32)[0]] = processed_ret
...
def test(opt):
  ...
  if opt.save_results:
    print('saving results to', opt.save_dir + '/save_results_{}{}.json'.format(
      opt.dataset, opt.dataset_version))
    json.dump(_to_list(copy.deepcopy(results)), 
              open(opt.save_dir + '/save_results_{}{}.json'.format(
                opt.dataset, opt.dataset_version), 'w'))
  dataset.run_eval(results, opt.save_dir)

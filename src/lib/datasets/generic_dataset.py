 ...
 #     if opt.tracking:
  ...
  def _load_pre_data(self, video_id, frame_id, sensor_id=1):
    ...
    if 'train' in self.split:
      img_ids = [(img_info['id'], img_info['frame_id']) \
          ...
          if abs(img_info['frame_id'] - frame_id) < self.opt.max_frame_dist and abs(img_info['frame_id'] - frame_id) > 0 and \
  ...
  def _get_pre_dets(self, anns, trans_input, trans_output):
    ...
    for ann in anns:
      ...
      if (h > 0 and w > 0):
        ...
        ct = np.array(
          [(bbox[0] + bbox[2]) / 2,  (bbox[1] + ((bbox[3] - bbox[1])/10))], dtype=np.float32)
        ct0 = ct.copy()
        conf = 1
  ...
  def _init_ret(self, ret, gt_det):
    ...
    ret['hm'] = np.zeros(
      ...
      np.float32)
    ret['head_hm'] = np.zeros(
      (self.opt.num_classes, self.opt.output_h, self.opt.output_w), 
      np.float32)
    ret['ind'] = np.zeros((max_objs), dtype=np.int64)
    ret['h_ind'] = np.zeros((max_objs), dtype=np.int64)
    ret['cat'] = np.zeros((max_objs), dtype=np.int64)
    ret['mask'] = np.zeros((max_objs), dtype=np.float32)
    ret['head_dist'] = np.zeros((max_objs), dtype=np.float32)
    ret['ids'] = np.zeros((self.max_objs, ), dtype=np.int64)
    ret['ids_mask'] = np.zeros((self.max_objs, ), dtype=np.int8)
    regression_head_dims = {
      'reg': 2, 'wh': 2, 'h_wh': 2, 'tracking': 2, 'ltrb': 4, 'ltrb_amodal': 4, 
  ...
  def _add_instance(
    ...
    ret['h_ind'][k] = head_int[1] * self.opt.output_w + head_int[0]
    ret['reg'][k] = head - head_int
    ret['reg_mask'][k] = 1
    ret['ids'][k] = ann['track_id']
    ret['ids_mask'][k] = 1
    draw_umich_gaussian(ret['hm'][cls_id - 1], ct_int, radius)
    draw_umich_gaussian(ret['head_hm'][cls_id - 1], head_int, radius)

    gt_det['bboxes'].append(
      np.array([head[0] - w / 2, head[1] - h / 10,
                head[0] + w / 2, 9*head[1] + h / 10], dtype=np.float32))
    ...
    if 'tracking' in self.opt.heads:
      if ann['track_id'] in track_ids:
        ...
        ret['tracking'][k] = pre_ct - head_int
    ...
    if 'ltrb_amodal' in self.opt.heads:
      ret['ltrb_amodal'][k] = \
        bbox_amodal[0] - head_int[0], bbox_amodal[1] - head_int[1], \
        bbox_amodal[2] - head_int[0], bbox_amodal[3] - head_int[1]

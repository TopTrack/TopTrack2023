...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
               num_classes=-1, dataset=None, down_ratio=4, opt=None):
    self.opt=opt
    self.ipynb = ipynb
    if not self.ipynb:
    ...
    if dataset == 'coco_hp':
      ...
      self.H = 375
        ...
        self.names = ['Pedestrian', 'Car', 'Cyclist']
        self.W = 1280
        self.H = 384
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'):
    ...
    if self.opt.show_track_color:
      track_id = int(conf)
      c = self.colors[track_id][0][0].tolist() if track_id < len(self.colors) else self._get_rand_color()
  ...
  def add_tracking_id(self, img_id, ct, tracking_id):

...
class Tracker(object):
  ...
  def init_track(self, results, images):
    for item in results:
      if item['score'] > self.opt.new_thresh:
        ...
        bbox = item['bbox']
        if not ('ct' in item):
          item['ct'] = [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 10]
  ...
  def step(self, results, public_det=None):
    ...
    if self.opt.hungarian:
      ...
      matched_indices = np.transpose(np.array(matched_indices))
    else:
...
def reid_matching(tracks, results, iou_dist, lambda_=0.98):
  cost_matrix = np.zeros((len(tracks), len(results)), dtype=np.float)
  matched_indices = []
  if cost_matrix.size == 0:
    return np.array(matched_indices, np.int32).reshape(-1, 2)

  id_features = np.asarray([det['id_features'] for det in results])
  track_features = np.asarray([track['id_features'] for track in tracks])
  cost_matrix = np.maximum(0.0, cdist(track_features, id_features, 'cosine'))
  fused_dist = lambda_*np.transpose(cost_matrix) + (1-lambda_)*iou_dist

  for i,row in enumerate(fused_dist):
    if np.min(row) < 1e14:
      j = np.argmin(row)
      fused_dist[:,j] = 1e18
      iou_dist[:,j] = 1e18
